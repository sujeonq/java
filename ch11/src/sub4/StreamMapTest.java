...
public class StreamMapTest {
	public static void main(String[] args) {
		...
		}).collect(Collectors.toList()); // 스트림으로 전송된 데이터 취합해 컬렉션으로 전환

		System.out.println(names);

		// flatMap 스트림
		List<List<String>> lists = List.of(Arrays.asList("apple", "banana"), Arrays.asList("cherry", "grape"),
				Arrays.asList("orange", "mango"));

		System.out.println(lists);

		List<String> fruits = lists.stream().flatMap((list) -> {
			return list.stream();
